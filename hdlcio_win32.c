...
if (masslen != 0) {
 ...
 if (res != (masslen-1)) {
   printf("\nToo short answer from the modem: %i bytes, was expected %i bytes\n",res+1,masslen);
...
if (*devname == '\0')
{
  printf("\n  SEARCH FOR A FIRMWARE PORT ... \n");

  
  do
  {
    find_port(&port_no, port_name);
  } while (result==1);
  if (result==0)
  {
      Sleep(3000);
...
{
    CloseHandle(hSerial);
    printf("\n! - Error when initializing COM port\n"); 
    exit(0); 
    //return -1;
}
...
if (pt == 0xa55aaa55) {
  printf("\n file %s has a header - it does not fit for the firmware\n",filename);
  exit(1);
}
